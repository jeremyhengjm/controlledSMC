function [cSMC,varargout] = cSMC_Resample(Parameters)
% Controlled Sequential Monte Carlo
    ...
    PerformLQG = Parameters.Demo; % compute LQG
    ...
    switch Approximator
        case 'quadratic'
            ...
            IndexC = (refA+2*d+1):refC;
            IndexCC = (refC+1):(refC+d);
            Indexd = (refA+d+1):(refA+2*d);
            
        case 'purequadratic'
            refA = 2 * d + 1;

            IndexAA = (d+2):refA;
            IndexCC = (refA+d+1):(refA+2*d);
            Indexd = (refA+1):(refA+d);
    ...
    function NewPolicy = ApproxDP(SMC,Policy)
        ...
        NewPolicy.C = zeros(M+1,d,d);
        NewPolicy.d = zeros(M+1,d);
        ...
        for k = (M+1):-1:1
            ...
            Value = - SMC.LogIncrementalWeight(:,k) - LogNormalisingFunc; % N x 1
            
            if (k == 1)
                DesignCurrent = x2fx(squeeze(SMC.Trajectory(:,:,1)),Approximator);
                BetaHat = (DesignCurrent' * DesignCurrent) \ (DesignCurrent' * Value); % (d^2/2 + 3d/2 + 1) x 1
                ...
                DesignCurrent = x2fx(squeeze(SMC.Trajectory(:,:,k)),Approximator);
                X = squeeze(SMC.Trajectory(:,:,k-1));
                DesignPrevious = x2fx(X(SMC.Ancestry(:,k),:),Approximator);
                DesignMatrix = [DesignCurrent DesignPrevious(:,2:end)]; % N x (d^2 + 3d + 1)
                BetaHat = (DesignMatrix' * DesignMatrix) \ (DesignMatrix' * Value); % (d^2/2 + 3d/2 + 1) x 1
                
                % Store parameters
                if ( d == 1 || strcmp(Approximator,'purequadratic') )
                    AHat = diag(BetaHat(IndexAA));
                    CHat = diag(BetaHat(IndexCC)); 
                else
                    AHat = diag(BetaHat(IndexAA)) + 0.5 * squareform(BetaHat(IndexA));
                    CHat = diag(BetaHat(IndexCC)) + 0.5 * squareform(BetaHat(IndexC));
                end
                bHat = BetaHat(Indexb)';
                dHat = BetaHat(Indexd)';
                eHat = BetaHat(Indexe);
                ...
                NewPolicy.C(k,:,:) = squeeze(Policy.C(k,:,:)) + CHat; 
                NewPolicy.d(k,:) = Policy.d(k,:) + dHat; 
                ...
                LogNormalisingFunc = - sum((X * CHat) .* X,2) - sum(bsxfun(@times,X,dHat),2) ... 
                                     - eHat + 0.5 * LogDetTheta - 0.5 * Policy.LogDetTheta(k-1) ... 
    ...
    % Compute exact value functions
    if strcmp(PerformLQG,'yes')
        Exact = ComputeLQG;
