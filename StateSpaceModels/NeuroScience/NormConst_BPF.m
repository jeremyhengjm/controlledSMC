% Examine variance of normalizing constant estimator on Demba's neuroscience example
clear
close 
clc
...
BPF_LogNormConst = zeros(nrep,ngrid);
...
for igrid = 1:ngrid
    ...
    for irep = 1:nrep
        irep
        BPF_LogNormConst(irep,igrid) = cSMC_Resample(Parameters, Initial, ConstructTransition([alpha sigmasqgrid(igrid)]), Observation, Purpose); 
    end
end    
toc

save('NormConst_BPF.mat','BPF_LogNormConst','BPF_N')
