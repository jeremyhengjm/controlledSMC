function cSMC = cSMC_Resample(Parameters, Initial, Transition, Observation, Purpose)
% Controlled Sequential Monte Carlo
    ...
    %                  Observation (struct)
    %                  Purpose (string)
    % Output argument cSMC depends on Purpose
...
%                 AHat = diag(BetaHat(IndexAA));
    ...
    switch Purpose
        ...
        case 'loglikelihood'
            ...
            cSMC = OutputSMC.LogNormConst(end);            
        ...
        case 'smoothing'
            ...
            cSMC = zeros(N,T+1);
            cSMC(:,end) = OutputSMC.Trajectory(TraceAncestor,1,end);
            for s = T:-1:1
                TraceAncestor = OutputSMC.Ancestry(TraceAncestor,s);
                cSMC(:,s) = OutputSMC.Trajectory(TraceAncestor,1,s);
            end
